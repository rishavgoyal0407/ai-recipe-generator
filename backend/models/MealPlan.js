...
class MealPlan {
    ...
    static async getWeeklyPlan(userId,weekStartDate){
        ...
        return await this.findByDateRange(userId,weekStartDate,endDate.toISOString().split("T")[0]);
    }
    ...
    static async getUpcoming(userId,limit=5){
        const result = await db.query(
            ...
            AND mp.meal_date < CURRENT_DATE + INTERVAL '7 days'
            ORDER BY mp.meal_date ASC,
            CASE mp.meal_type
            WHEN 'breakfast' THEN 1
            WHEN 'lunch' THEN 2
